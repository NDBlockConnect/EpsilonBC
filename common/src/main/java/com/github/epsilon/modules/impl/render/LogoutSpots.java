...
public class LogoutSpots extends Module {
    ...
    private void onRender2D(Render2DEvent.Level event) {
        ...
        for (LogoutPlayer player : logoutCache.values()) {
            ...
            Vector3f projectedAnchor = WorldToScreen.getWorldPositionToScreen(anchor);
            if (projectedAnchor == null) continue;

            float textScale = calcAnchorScale(anchor) * 0.4f;
    ...
    /**
     * 26.1.x 兼容：近似 Private WorldToScreen.calcScale 的透视缩放。
     */
    private float calcAnchorScale(Vec3 pos) {
        Camera camera = mc.gameRenderer.getMainCamera();
        double dist = camera.position().distanceTo(pos);
        float depth = (float) Math.max(dist, 1.0);
        float projectionYScale = 1.0f / (float) Math.tan(Math.toRadians(camera.getFov()) * 0.5);
        float halfHeight = LuminRenderSystem.getScaledHeight() * 0.5f;
        return projectionYScale * halfHeight / depth;
    }
